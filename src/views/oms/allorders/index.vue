  ...
  <div class="app-container">
    <el-row :gutter="20">
      ...
      <el-col :span="24" :xs="24">
        ...
        >
          <template v-slot>
            <vxe-form :data="bsTypeForm" title-align="right" title-width="60">
              <vxe-form-item
                field="bstype"
                span="24"
                :item-render="{ name: '$radio', options: [{ value: 1, label: '出口订单' }, { value: 2, label: '进口订单' }] }"
              ></vxe-form-item>
              <vxe-form-item align="right" span="24">
                <template v-slot>
                  <vxe-button type="submit" status="primary" @click="searchEvent">确定</vxe-button>
                </template>
              </vxe-form-item>
            </vxe-form>
          </template>
...
import OrderDetailExport from '../shippingexport/orderdetail/index.vue';
import OrderDetailImports from '../shippingimports/orderdetail/index.vue';
import { listBusinessOrder, exportData } from '@/api/oms/businessOrder';
...
export default {
  ...
  methods: {
    ...
    searchEvent() {
      ...
      this.type = 1;
      this.showType = 1;
      if (this.bsTypeForm.bstype == 1) {
        this.genre = 1;
      } else if (this.bsTypeForm.bstype === 2) {
        this.genre = 2;
      }
      this.bsTypeFormVisible = false;
    },
    // 复制订单
    handleCopy() {
    ...
    goBack() {
      ...
      let addData = this.$refs.headerChild.$refs.makingChestChild.$refs.zxdTable.getInsertRecords()
        .length;
      let delData = this.$refs.headerChild.$refs.makingChestChild.$refs.zxdTable.getRemoveRecords()
        .length;
      let updateData = this.$refs.headerChild.$refs.makingChestChild.$refs.zxdTable.getUpdateRecords()
        .length;
      if (
