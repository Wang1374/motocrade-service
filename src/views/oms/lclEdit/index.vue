<template>
  ...
  >
                  ...
                  <el-col :span="22">
                    <el-form-item label="门点简称" :rules="[{ required: true }]">
                      <el-select
                        ...
                        placeholder="请选择"
                        @visible-change="v => visibleChange(v, 'doorsSelect', i, addDoorsClick, '添加门点')"
                      >
                        <el-option
    ...
    <add-doors
      ref="addDriver"
      v-if="addDoorsVisible"
      :visible.sync="addDoorsVisible"
      :ctId="childCustomerId"
      @doors-success="onDoorsSuccess"
    ></add-doors>
  </el-dialog>
</template>
<script>
import { selectBottomAction } from '@/utils/mixins.js';
import AddDoors from '../addDoors/index.vue';
...
export default {
  ...
  mounted() {
    if (this.childDoorsList.length > 0) {
      this.doorsOptions = this.childDoorsList;
    }
  ...
  methods: {
    ...
    submitLcl: function () {
      ...
      for (var i = 0, len1 = this.lclTabs.length; i < len1; i++) {
        ...
        for (var j = 0, len2 = lclList.length; j < len2; j++) {
          if (validatenull(lclList[j].door)) {
            ...
            for (var k = 0, len3 = hairBodyList.length; k < len3; k++) {
              ...
              productName = hairBodyList[k].productName;
            }
          }
        }
        ...
        this.lclTabs[i].productName = productName;
      }
