<template>
  <div class="eir">
    <div v-if="showType ==0">
      ...
      >
        <el-carousel-item v-for="(item, index) in eirImgList" :key="index" class="imageFlex">
          <el-image
            fit="fit"
            class="eirpic"
            :src="item"
            :style="'width:'+(myWidth)+'px'"
            :preview-src-list="eirImgList"
          >
        ...
        <div class="items">
          ...
          >
            <i
              ...
              @click="downloadEirPicture()"
            ></i>
          </el-tooltip>
        </div>
        <div class="items" v-show="eirImgList.length > 0">
          <i class="el-icon-arrow-left demo" @click="eirPre()" v-show="eirImgList.length > 1"></i>
          <i class="el-icon-plus demo" @click="addEirPicture()"></i>
          <i class="el-icon-minus demo" @click="subEirPicture()"></i>
          <i class="el-icon-refresh-right demo" @click="revolveEirPicture()"></i>
          <i class="el-icon-arrow-right demo" @click="eirNext()" v-show="eirImgList.length > 1"></i>
    ...
    <div v-if="showType==1">
          ...
          <el-form ref="form" :model="form">
            <el-form-item>
              <el-upload
                ...
                :headers="headers"
                :file-list="eirUrlArray"
                :on-preview="handlePictureCardPreview"
                :on-remove="handleRemove"
                :before-upload="beforeAvatarUpload"
                :on-success="handleEirSuccess"
      ...
      <div style="border-top: 1px solid #ccc; height: 36px" class="bottomItem">
        <div class="items"></div>
        <div class="items">
          <i class="el-icon-arrow-left demo" v-show="this.eirUrlArray.length > 1"></i>
...
export default {
  ...
  watch: {
    eirImgList(newList, oldList) {
      this.eirImgList = newList;
      if (this.eirImgList.length == 0) {
        this.showType = 1;
      } else {
        this.showType = 0;
      }
      // 图片地址
      var img_url = this.eirImgList[0];
      // 创建对象
      var img = new Image();
      // 改变图片的src
  ...
  methods: {
    ...
    handleEirSuccess(res, file) {
      this.showType = 0;
      this.eirImgList[0] = res.data.previewUrl;
    ...
    handleClick(tab, event) {
      this.$refs.cardEirShow.activeIndex = 0;
      var image =
        document.getElementsByClassName('eirpic')[
          this.$refs.cardEirShow.activeIndex
        ];
    ...
    eirPre() {
      //如果不是第一张
      if (this.$refs.cardEirShow.activeIndex > 0) {
        //箱门照片
        var image =
          document.getElementsByClassName('eirpic')[
            this.$refs.cardEirShow.activeIndex
        ...
        image.style.width = this.widths + 'px';
        image.style.transform = 'rotate(' + 0 + 'deg)';
        this.$refs.cardEirShow.prev();
      }
      this.index = this.$refs.cardEirShow.activeIndex;
    },
    //箱门下一张
    eirNext() {
      //如果不是最后一张
      if (this.$refs.cardEirShow.activeIndex < this.eirImgList.length - 1) {
        var image =
          document.getElementsByClassName('eirpic')[
            this.$refs.cardEirShow.activeIndex
          ];
    ...
    //下载图片
    downloadEirPicture() {
      var index = this.$refs.cardEirShow.activeIndex;
      var url = this.eirImgList[index];
