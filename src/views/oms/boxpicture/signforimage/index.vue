  ...
  <div class="signFor">
    <div v-if="showType ==0">
      ...
      >
        <el-carousel-item v-for="(item, index) in signForImgList" :key="index" class="imageFlex">
          <el-image
            ...
            :preview-src-list="signForImgList"
          >
      ...
      <div style="border-top: 1px solid #ccc; height: 36px" class="bottomItem">
        ...
        <div class="items" v-show="signForImgList.length > 0">
          <i
            class="el-icon-arrow-left demo"
            @click="signForPre()"
            v-show="signForImgList.length > 1"
          ></i>
    ...
    <div v-if="showType==1">
          ...
          <el-form ref="form" :model="form">
            <el-form-item>
              <el-upload
                ...
                :class="{ hide: signForHide }"
                list-type="picture-card"
                :headers="headers"
                :file-list="signForUrlArray"
                :on-preview="handlePictureCardPreview"
                :on-remove="handleRemove"
                :before-upload="beforeAvatarUpload"
                :on-success="handleSignForSuccess"
                :on-error="imgUploadError"
                :on-change="signForChange"
              >
      ...
      <div style="border-top: 1px solid #ccc; height: 36px" class="bottomItem">
        <div class="items"></div>
        <div class="items">
          <i class="el-icon-arrow-left demo" v-show="this.signForUrlArray.length > 1"></i>
...
export default {
  ...
  watch: {
    signForImgList(newList, oldList) {
      this.signForImgList = newList;
      if (this.signForImgList.length == 0) {
        this.showType = 1;
      } else {
        this.showType = 0;
      }
  ...
  methods: {
    ...
    handleSignForSuccess(res, file) {
      this.showType = 0;
      this.signForImgList[0] = res.data.previewUrl;
    ...
    deletePicture() {
      this.showType = 1;
      this.signForHide = false;
      var arr = [];
      this.$emit('signForImgChange', arr);
    },
    ///////////////////////////////////////////////////
    // 取消按钮
    cancel() {
    ...
    handleClick(tab, event) {
      this.$refs.cardSignForShow.activeIndex = 0;
      var image =
        document.getElementsByClassName('signForpic')[
