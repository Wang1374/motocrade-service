...
import { upload } from "@/api/system/attachment";
  ...
  methods: {
    ...
    uploadImg() {
      this.$refs.cropper.getCropBlob(data => {
        ...
        // 先上传，拿出返回结果中的url 与 id
        upload(formData).then(response => {
          if (response.data.code === 200) {
            var data = response.data.data;
            // 修改头像
            this.userInfo.avatarId = data.id;
            uploadAvatar(this.userInfo).then(response => {
              if (response.data.code === 200) {
                this.open = false;
                this.msgSuccess("修改成功");
                // 更新localStorage
                store.commit("SET_AVATAR", data.previewUrl);
