  ...
  <div>
    <el-row :gutter="20">
      ...
      <el-col :span="24" :xs="24">
        ...
        <el-table :data="doorsList" @row-dblclick="updateDate" highlight-current-row>
...
import {
  ...
  updateDoors,
  delDoors
} from '@/api/distribute/clientdoors';
...
export default {
  ...
  methods: {
    ...
    updateDate(row) {
      ...
      this.originalDoorAs = this.form.doorAs;
      this.open = true;
      this.title = '修改门点';
    },
    /** 删除按钮操作 */
    handleDelete(row) {
      this.$confirm('是否确认删除门点为"' + row.doorAs + '"的数据项?', '警告', {
        ...
        .then(function () {
          var params = {
            id: row.id,
            belongCompaniesId: row.belongCompaniesId
          };
          return delDoors(params);
        })
        .then(() => {
          this.getList();
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          if (this.form.id != undefined) {
            //修改
            this.form.originalDoorAs = this.originalDoorAs;
