  ...
  <div style="margin-right: 40px">
    <el-form ref="form" :model="form" :rules="rules">
      ...
      <el-row style="margin-top: 10px" class="bt">
        <el-button type="primary" @click="submitOrder" :disabled="btOrderabled">提 交 修 改</el-button>
      </el-row>
...
import { editMakingCenter } from '@/api/distribute/businessCenter';
import LclEdit from '@/views/distribute/singlecenter/placeorder/lclEdit';
...
export default {
  ...
  created() {
    this.form = this.editRowInfo;
    // 赋值 船名航次 过滤
    if (this.editRowInfo.vesselAndVoyage0 !== null) {
      var last = this.editRowInfo.vesselAndVoyage0.lastIndexOf('/');
      this.$set(
        this.form,
        'shipsName',
        this.editRowInfo.vesselAndVoyage0.substring(0, last)
      );

      this.$set(
        this.form,
        'voyage',
        this.editRowInfo.vesselAndVoyage0.substring(
          last + 1,
          this.editRowInfo.vesselAndVoyage0.length
        )
      );
    }
    //赋值
    this.$set(this.form, 'portOfLoading', this.editRowInfo.portOfLoading0);
    this.$set(this.form, 'portOfDischarge', this.editRowInfo.portOfDischarge0);
    this.$set(this.form, 'placeOfDelivery', this.editRowInfo.placeOfDelivery0);
    this.$set(this.form, 'dock', this.editRowInfo.dock0);
    this.$set(this.form, 'customerNum', this.editRowInfo.customerNum0);
    //赋值提单号
    // 赋值提单号
    if (!validatenull(this.editRowInfo.blNos0)) {
      this.blNoArray = [];
      let array = this.editRowInfo.blNos0.split(',');
    ...
    this.$set(this.form, 'remark', this.editRowInfo.remark);

    if (
      this.editRowInfo.appendixUrl == null ||
      this.editRowInfo.appendixUrl == ''
    ) {
      this.appendixArray = [];
    } else {
      this.appendixArray.push({
        name: this.editRowInfo.appendixName,
        url: this.editRowInfo.appendixUrl
      });
    }
    // this.$set(this.form,"appendixUrl",this.editRowInfo.appendixUrl)
    // this.$set(this.form,"appendixName",this.editRowInfo.appendixName)
    this.getMccList();
  },
  data() {
    return {
      ...
      childCustomerId: '',
      validRules: {
        boxPile: [{ required: true, message: '箱型必须填写' }]
      },
      //
      loadingList: false,
      //做箱mcIds
  ...
  methods: {
    /** 查询做箱信息列表 */
    getMccList() {
      if (this.editRowInfo.id !== '') {
        this.loadingList = true;
        getMakingChestCenterList(this.editRowInfo.id).then((response) => {
          ...
          this.makingList = data;
          //做箱ids
          this.mcIds = data.map((item) => {
            return item.id;
          });
    ...
    async submitOrder() {
      //做箱数据
      this.makingListData = this.$refs.zxdTable.getTableData().fullData;
      ...
      if (errMap) {
        ...
        //未上传附件
        if (validatenull(this.form.appendixUrl)) {
          this.$refs['form'].validate((valid) => {
            if (valid) {
              ...
              if (this.blNoArray[this.blNoArray.length - 1].blNo == '') {
              ...
              } else if (this.makingListData.length == 0) {
                this.$message({
                  type: 'warning',
                  message: '请填写做箱信息'
                });
                ...
                var flag = false;
                var number;
                for (var i = 0; i < this.makingListData.length - 1; i++) {
                  for (var j = i + 1; j < this.makingListData.length; j++) {
                    if (
                      this.makingListData[i].caseNumber == null &&
                      this.makingListData[j].caseNumber == null
                    ) {
                      continue;
                    }
                    if (
                      this.makingListData[i].caseNumber.toString() ===
                      this.makingListData[j].caseNumber.toString()
                    ) {
                      number = this.makingListData[i].caseNumber;
                      flag = true;
                      break;
                    }
                  }
                ...
                if (flag) {
                  ...
                  //封装数据
                  this.composeNotUrl(this.makingListData);

                  //禁用
                  this.btOrderabled = true;
                  // 修改下单
                  editMakingCenter(this.form).then((response) => {
          ...
          //已经上传附件
          //封装数据
          this.composeIsUrl(this.makingListData);
          //禁用
          this.btOrderabled = true;
          //添加做箱
          editMakingCenter(this.form).then((response) => {
    ...
    composeNotUrl(makingList) {
      ...
      //3拼接船名航次
      this.form.vesselAndVoyage = this.form.shipsName + '/' + this.form.voyage;
    },
    //已经上传附件提交订单封装数据
    composeIsUrl(makingList) {
      ...
      if (this.form.shipsName != undefined && this.form.voyage != undefined) {
        this.form.vesselAndVoyage =
          this.form.shipsName + '/' + this.form.voyage;
      }
    },
    //封装基础表单数据
    composeBaseData(makingList) {
      ...
      (this.form.singleUser = this.userInfo.name),
        (this.form.singlePhone = this.userInfo.identifier),
        (this.form.companyName = this.company.companyName);
      //做箱id数组
      this.form.mcIds = this.mcIds;
    },
