...
export default {
  ...
  created() {
    this.form = this.editImportRowInfo;
    // 赋值 船名航次 过滤
    if (this.editImportRowInfo.vesselAndVoyage0 !== null) {
      var last = this.editImportRowInfo.vesselAndVoyage0.lastIndexOf('/');
      this.$set(
        this.form,
        'shipsName',
        this.editImportRowInfo.vesselAndVoyage0.substring(0, last)
      );

      this.$set(
        this.form,
        'voyage',
        this.editImportRowInfo.vesselAndVoyage0.substring(
          last + 1,
          this.editImportRowInfo.vesselAndVoyage0.length
        )
      );
    }
    //赋值
    this.$set(
      this.form,
      'portOfLoading',
      this.editImportRowInfo.portOfLoading0
    );
    this.$set(
      this.form,
      'portOfDischarge',
      this.editImportRowInfo.portOfDischarge0
    );
    this.$set(
      this.form,
      'placeOfDelivery',
      this.editImportRowInfo.placeOfDelivery0
    );
    this.$set(this.form, 'dock', this.editImportRowInfo.dock0);
    this.$set(this.form, 'customerNum', this.editImportRowInfo.customerNum0);
    //赋值提单号
    // 赋值提单号
    if (!validatenull(this.editImportRowInfo.blNos0)) {
      this.blNoArray = [];
      let array = this.editImportRowInfo.blNos0.split(',');
    ...
    this.$set(this.form, 'remark', this.editImportRowInfo.remark);

    if (
      this.editImportRowInfo.appendixUrl == null ||
      this.editImportRowInfo.appendixUrl == ''
    ) {
      this.appendixArray = [];
    } else {
      this.appendixArray.push({
        name: this.editImportRowInfo.appendixName,
        url: this.editImportRowInfo.appendixUrl
      });
    }
    this.getMccList();
  },
  ...
  methods: {
    /** 查询做箱信息列表 */
    getMccList() {
      if (this.editImportRowInfo.id !== '') {
        this.loadingList = true;
        getMakingChestCenterList(this.editImportRowInfo.id).then((response) => {
              ...
              if (this.blNoArray[this.blNoArray.length - 1].blNo == '') {
                ...
                for (var i = 0; i < this.makingListData.length - 1; i++) {
                  for (var j = i + 1; j < this.makingListData.length; j++) {
                    ...
                    if (
                      this.makingListData[i].caseNumber ===
                      this.makingListData[j].caseNumber
                    ) {
