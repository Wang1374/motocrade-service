        ...
        >
          <template v-slot:edit="scope">
            <el-select
              ...
              no-data-text="无匹配数据"
              placeholder="请选择"
              @visible-change="v => visibleChange(v, 'fleetName', 0, addBtypeClick, '添加往来单位')"
            >
              <el-option
        ...
        <vxe-table-column title="操作" fixed="right" width="120">
          <template slot-scope="scope">
            <el-button
              ...
              type="text"
              style="color: #f56c6c"
              :disabled="getDisabled2(costData.ptysState, scope.row.expenseStatus)"
...
export default {
  ...
  data() {
    return {
      ...
      validRules: {
        fleetName: [{ required: true, message: '往来单位必须填写' }],
  ...
  methods: {
    ...
    addRow(row) {
      ...
      let record = {
        ...
        mcId: this.costData.mcId,
        btypeId: this.company.id,
        btype: this.company.companyName,
        partner: undefined,
        payer: undefined,
        payItems: undefined,
        unitPrice: undefined,
    ...
    //   if (value !== undefined) {
    //     obj = list.find(item => {
    //       return item.feeCode === value;
    //     });
    //   }
    //   return obj == null ? null : obj.feeName;
    // },
    // 总价
    countAmount(row) {
    ...
    btypeIdChange(row) {
      var newArr = this.customerOptions.filter(function (obj) {
        return obj.value == row.fleetName;
      });
      if (newArr.length > 0) {
        row.fleetId = newArr[0].id;
    ...
    getDisabled(invoiceState, expenseStatus) {
      // 查询  let state = checkPermi(["cost:detailed:receipts"]); 权限状态 进行禁用
    ...
    getDisabled2(ptysState, expenseStatus) {
      let corporateIdentify = store.getters.userInfo.corporateIdentify;
              ...
              this.$confirm('‘应收已全’ 将无法新增与删除, 是否继续?', '提示', {
                ...
                .then(() => {
                  ...
                  var params = {
                    ...
                    ysState: -1,
                    yfState: -1,
                    ptysState: this.costData.ptysState
                  };
