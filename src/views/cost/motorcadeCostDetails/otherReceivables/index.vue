  ...
  <el-row>
    ...
    <el-col :span="22" :offset="1">
      ...
      >
        ...
        <vxe-table-column field="invoiceNo" title="发票号1" :edit-render="{name: 'input'}" width="110">
          <template v-slot:edit="{ row }">
...
export default {
  ...
  methods: {
    ...
    isNotChange() {
      //1 长度不同
      var insert = this.$refs.qtysTable.getInsertRecords().length;
      var update = this.$refs.qtysTable.getUpdateRecords().length;
      var remove = this.$refs.qtysTable.getRemoveRecords().length;
    ...
    getList() {
      ...
      getCostList(params).then((response) => {
        this.$refs.qtysTable.loadData(response.data);
        this.loadingList = false;
      });
    },
    // 添加行
    addRow(row) {
      let qtysTable = this.$refs.qtysTable;
      let record = {
        id: undefined,
      ...
      qtysTable.insertAt(record, row).then(({ row }) => {
        qtysTable.setActiveRow(row);
      });
    },
    // 删除行
    deleteRow(row) {
      this.$XModal.confirm('您确定要删除该数据?').then((type) => {
        if (type === 'confirm') {
          this.$refs.qtysTable.remove(row);
        }
      });
    },
    // 总价
        ...
        columns.map((column, columnIndex) => {
          ...
          if (['totalPrice'].includes(column.property)) {
            ...
            if (this.$route.name == 'Analysis') {
              store.dispatch('setQtysTotalAnalysis', total).then(() => {});
            } else {
              store.dispatch('setQtysTotalCost', total).then(() => {});
            }
            return total;
          }
          return null;
    ...
    async submitTable() {
      ...
      const errMap = await this.$refs.qtysTable
        .fullValidate()
        .catch((errMap) => errMap);
      if (errMap) {
        let msgList = [];
        ...
        let fullData = this.$refs.qtysTable.getTableData().fullData;
