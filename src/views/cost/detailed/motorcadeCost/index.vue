      ...
      <el-col :span="24" :xs="24">
        <div v-show="showCostType === 0">
          ...
          >
            ...
            <el-form-item
              prop="expenseStatus"
              v-if="queryParams.costTypes == 1 || queryParams.costTypes == 3 || queryParams.costTypes==0"
            >
              <el-select
          ...
          <el-row :gutter="10" class="mb8">
            <el-col :span="1.5">
              <el-button
                ...
                size="mini"
                @click="batchConfirm"
                v-hasPermi="['cost:detailed:enter']"
              >批量确认</el-button>
            </el-col>
            <el-col :span="1.5">
              <el-button
          ...
          >
            ...
            >
              <template slot-scope="scope">
                ...
                <span v-else-if="item.rowField ==='payItems'">
                  {{scope.row.payItems}}
                  <el-badge class="mark" :value="scope.row.invoiceUrlNum" />
                </span>
                <span
        ...
        <el-dialog
          ...
          width="400px"
        >
          <el-form ref="fphForm" :model="fphForm" :rules="rules" label-width="80px">
            <el-form-item label="发票号" prop="invoiceNo">
              <el-input v-model="fphForm.invoiceNo" placeholder="请输入发票号"></el-input>
            </el-form-item>
          </el-form>
          <span slot="footer" class="dialog-footer">
            <el-button @click="fphDialogVisible = false">取 消</el-button>
            <el-button type="primary" @click="fphSubmitForm">确 定</el-button>
          </span>
        </el-dialog>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getCostListPage(this.queryParams).then((response) => {
        response.data.list.forEach((item) => {
          if (!validatenull(item.invoiceUrl)) {
            var num = item.invoiceUrl.split(',').length;
            item.invoiceUrlNum = num;
          }
        });
    ...
    batchConfirm() {
      ...
      if (this.multipleSelection.length !== 0) {
        ...
        for (var i = 0; i < list.length; i++) {
          if (this.multipleSelection[i].expenseStatus == 0) {
            list[i].affirmState = true;
            list[i].expenseStatus = 1;
            state = true;
          } else {
            state = false;
            break;
    ...
    enterInvoice() {
      if (this.multipleSelection.length !== 0) {
        ...
        for (var i = 0; i < this.multipleSelection.length; i++) {
          // 未开票前 可输入发票号
          if (this.multipleSelection[i].expenseStatus < 2) {
            state = true;
          } else {
            state = false;
            break;
        ...
        if (state) {
          this.fphDialogVisible = true;
        } else {
          this.$message({
            message: '请选择‘未开票’的账单进行操作',
            type: 'warning'
          });
        }
      }
    ...
    // 录入发票号保存
    fphSubmitForm() {
      this.$refs['fphForm'].validate((valid) => {
        if (valid) {
          for (var i = 0; i < this.multipleSelection.length; i++) {
            this.multipleSelection[i].invoiceNo = this.fphForm.invoiceNo;
          }
